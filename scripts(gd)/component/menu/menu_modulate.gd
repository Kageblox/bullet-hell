@tool
class_name MenuModulateComponent
extends Control
## Component Script that modulates when the connected menu opens or closes.

#region Variables

@export var invisible_color: Color = Color(1,1,1,0)

var _visible_color: Color = Color(1,1,1,1)
@export var visible_color: Color:
	get:
		return _visible_color
	set(value):
		_visible_color = value
